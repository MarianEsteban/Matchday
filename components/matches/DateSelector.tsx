...
export function DateSelector({ selectedDate, onSelectDate }: DateSelectorProps) {
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-1.5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70 dark:shadow-black/20">
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
        <button
          type="button"
          onClick={() => onSelectDate(shiftLocalDateKey(selectedDate, -1))}
          className="flex h-8 w-8 items-center justify-center rounded-md border border-stone-200 bg-stone-50 text-sm font-bold text-zinc-800 transition hover:border-stone-400 hover:bg-white dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 dark:hover:border-zinc-600"
            ...
            {dates.map((dateKey) => {
              ...
              return (
                <button
                  ...
                  className={`relative min-w-[3.7rem] rounded-md border px-2 py-1.5 text-center transition ${
                    isSelected
                ...
                >
                  <span className="block text-[0.68rem] font-semibold">{weekday}</span>
                  <span className="mt-0.5 block text-sm font-black leading-none">{day}</span>
                  <span className="mt-0.5 block text-[0.62rem] font-semibold opacity-70">{month}</span>
                  {isToday ? <span className={`mt-1 inline-block rounded px-1 py-0.5 text-[0.55rem] font-semibold ${isSelected ? "bg-emerald-400 text-zinc-950" : "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"}`}>{t("today")}</span> : null}
                </button>
              );
            })}
          </div>
