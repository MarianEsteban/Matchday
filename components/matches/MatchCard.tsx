...
function TeamCell({ team, align = "left" }: { team: Team; align?: "left" | "right" }) {
  return (
    <div className={`flex min-w-0 items-center gap-2 ${align === "right" ? "flex-row-reverse text-right" : ""}`}>
      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-stone-200 bg-white shadow-sm shadow-stone-200/50 dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-black/20 sm:h-8 sm:w-8">
        <Image src={team.crestUrl} alt={`${team.name} crest`} width={24} height={24} className="h-5 w-5 object-contain sm:h-6 sm:w-6" />
      </span>
      <span className="min-w-0 truncate text-[0.82rem] font-bold text-zinc-950 dark:text-zinc-50 sm:text-sm">{team.name}</span>
    </div>
  );
}
...
export function MatchCard({ match }: MatchCardProps) {
  ...
  return (
    ...
    >
      <article className={`relative grid grid-cols-[3.35rem_minmax(0,1fr)_auto] items-center gap-2 px-2.5 py-2.5 transition hover:bg-stone-50/90 dark:hover:bg-zinc-900/80 sm:grid-cols-[4.4rem_minmax(0,1fr)_auto] sm:px-4 ${isLive ? "bg-emerald-50/75 dark:bg-emerald-950/16" : ""} ${isHighlighted ? "before:absolute before:inset-y-2 before:left-0 before:w-1 before:rounded-r-full before:bg-amber-400/80" : ""}`}>
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_3.4rem_minmax(0,1fr)] items-center gap-2 sm:grid-cols-[minmax(0,1fr)_4rem_minmax(0,1fr)]">
          ...
          <span className={`rounded-lg px-1.5 py-1 text-center text-sm font-black tracking-wide ${match.score ? "text-zinc-950 dark:text-white" : "text-stone-400 dark:text-zinc-500"}`}>
            <span className="block leading-none">{scoreText}</span>
            {penaltiesText ? <span className="mt-0.5 block text-[0.55rem] font-black uppercase tracking-wide text-amber-700 dark:text-amber-300">{penaltiesText}</span> : null}
          </span>
